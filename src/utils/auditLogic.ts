...
export interface MockLlmInput {
  task: "test_connection" | "generate_audit_section" | string;
  provider?: string;
  evidence_refs?: string[];
  section_type?: string;
}

export function executeMockLlmCall(input: MockLlmInput): any {
  if (input.task === "test_connection") {
    return {
      ok: true,
      provider: input.provider || "mock",
      message: "Mock provider is working. No external LLM call was made."
    };
  }

  if (input.task === "generate_audit_section") {
    return {
      section_id: `mock_${input.section_type || "work_system_coherence"}`,
      title: input.section_type === "work_system_coherence" ? "Work System Coherence" : "Workflow Efficiency Analysis",
      narrative: `Mock narrative based on provided evidence references: ${(input.evidence_refs || []).join(", ") || "EV-001"}.`,
      evidence_refs: input.evidence_refs || ["EV-001"],
      assumptions: ["ASSUMPTION: generated by mock provider."],
      kpis: ["workflow_variance", "issue_type_variance"],
      benchmarks: ["TBD"],
      confidence: "medium",
      limitation: "Mock output. No external model was called.",
      counter_hypothesis: "Observed variance may be legitimate if teams serve different service classes.",
      visualization: {
        type: "bar_chart",
        reason: "Shows variance across work systems."
      },
